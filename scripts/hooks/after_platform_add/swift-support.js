/*
 * This method adds all the needed config to enable Swift plugins in a Cordova project.
 *
...
function addSwiftSupport(context, xcodeProject, callback) {
  ...
  platformMetadata.getPlatformVersions(projectRoot).then((platformVersions) => {
    ...
    let config = getConfigParser(context, path.join(projectRoot, 'config.xml'));
    ...
    platformVersions.forEach((platformVersion) => {
      if (platformVersion.platform === 'ios') {
        console.log(`iOS Platform Version = ${platformVersion.version}`);
    ...
    console.log(`Bridging Header Path = ${bridgingHeaderPath}`);
    ...
    glob('**/*Bridging-Header*.h', { cwd: pluginsPath }, (error, files) => {
      ...
      console.log('Updating Bridging Headers...');
      ...
      console.log('...done Updating Bridging Headers');
      ...
      console.log('...done adding Swift support');
      callback();
    });
  });
}
...
module.exports = { addSwiftSupport: addSwiftSupport };
